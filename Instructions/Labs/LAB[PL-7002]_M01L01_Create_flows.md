...
1. **[条件]** ステップを選んで、フロー ステップを展開します。

    ![フロー実行ステップのスクリーンショット。](../media/flow-run-steps.png)
...
1. **[Expand condition]\(条件の展開\)** を選びます
...
1. **[Let's automate something.What should it do?]\(何かを自動化しましょう。何をすべきでしょうか?\)** で、「`Every day send me an email with the daily summary from MSN Weather`」と入力します。
...
1. [すべてのトリガーを検索する] に `email` と入力します。
...
1. **[Notification Text]\(通知テキスト\)** フィールドを選び、[動的なコンテンツ] アイコンを選びます。
