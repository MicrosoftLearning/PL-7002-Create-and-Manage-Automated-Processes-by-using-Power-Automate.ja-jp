...
lab:
  ...
  description: このラボでは、クラウド フローを作成します。
  duration: 30 minutes
  level: 100
  islab: true
---
...
1. **[位置情報を知らせる]** というポップアップが表示されたら **[許可する]** を選びます。

    ![[場所] ポップアップのスクリーンショット。](../media/allow-location.png)

   > [!NOTE]
   > "このフローを実行するには位置情報サービスを有効にする必要がある"** というエラーが表示された場合は、コンピューターの **[設定] -> [プライバシーとセキュリティ] -> [位置情報]** で **[位置情報サービス]** と **[アプリに位置情報へのアクセスを許可する]** を有効にしてください。
...
1. **[本文]** フィールドで、既存のコンテンツを削除します。

1. **[ダイナミック値]** アイコンを選択するか、「`/`」と入力して**動的コンテンツを挿入します**。

1. 検索フィールドに「`daily summary`」と入力します。
...
> [!NOTE]
> 状態コード **"Forbidden"** と詳細 **"ConnectionAuthorizationFailed"** が保存後に表示される場合は、手順 10 から 13 を実行して新しい接続を作成します。 エラーが表示されない場合は、手順 14 に進みます。

10. **[今日の予測の取得]** アクション、**[接続参照の変更]**、**[新規追加]**、**[新規作成]** を順に選択します。

1. **[プロファイルの取得]** アクション、**[接続参照の変更]**、**[新規追加]**、**[サインイン]** を順に選択します。

1. **[メールの送信]** アクション、**[接続参照の変更]**、**[新規追加]**、**[サインイン]** を順に選択します。

1. 各アクションの新しい接続を作成した後、**[保存]** を選択します。
...
1. 検索で「`post message`」と入力します。

    ![アクションの検索のスクリーンショット。](../media/search-action.png)

1. **Microsoft Teams** の下の **[チャットまたはチャネルにメッセージを投稿する]** を選択します。
...
1. **[投稿先]** には、**[フロー ボットとのチャット]** を選択します。

1. **[受信者]** にテナント ユーザー ID を入力します。

1. **[メッセージ]** フィールドに「`/`」と入力し **[動的コンテンツの挿入]** を選択します。
...
1. **[すべて検出]** を選択し、[監視]セクションで**クラウド フロー アクティビティ**を検索します。
    ...
    ![Power Automate ポータルのその他のオプションのスクリーンショット。](../media/discover-cloud-flow-activity.png)
