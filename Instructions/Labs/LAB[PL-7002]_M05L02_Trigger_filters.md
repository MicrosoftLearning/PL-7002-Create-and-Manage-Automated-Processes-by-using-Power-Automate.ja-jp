...
> [!NOTE]
> トリガー名の編集で問題が発生した場合は、**Copilot** を使用して名前を変更します。  
> **[Copilot]** を選択し、Copilot チャットで次のプロンプトを入力します: `Rename the trigger to Opportunity changed`。
...
1. トリガー ステップの下にある **+** アイコンを選択し、アクションを追加します。

1. 検索ボックスに、`email` と入力します。
...
1. **[フィルター行]** フィールドを選択し、前の演習の **論理名** を使用して「`cr977_opportunitystatus eq 3`」を入力します。
...
1. 新しいブラウザー タブで Power Apps Maker Portal (`https://make.powerapps.com`) に移動します。
...
1. **[営業案件の列とデータ]** の下にある任意のレコードを選択し、**[営業案件の状態]** を **[クローズ]** に変更します。

1. Power Apps Maker Portal で、ブラウザー ウィンドウの左上にある**アプリ起動ツール**を選択し、**[Outlook]** を選択します。

1. 電子メールによる通知が届いたことを確認します。